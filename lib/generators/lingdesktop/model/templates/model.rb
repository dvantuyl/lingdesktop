class <%= class_name %> < RDF_Resource
  ...
  def self.gen_uri_esc
    ...
    "http://purl.org/linguistics/lingdesktop/<%= plural_name %>/#{uuid}".uri_esc
  end  
  
  
  def self.type
    RDF_Resource.find_or_create(:uri_esc => RDF::GOLD.<%= class_name %>.uri_esc)
  end
  ...
  def self.create_in_context(context_node, args = {})
    ...
    node = <%= class_name %>.create(:uri_esc => self.gen_uri_esc)
