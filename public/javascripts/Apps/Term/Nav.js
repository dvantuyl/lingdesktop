...
Term.Nav = Ext.extend(Desktop.App, {
	...
	initComponent : function() {
		...
		//setup mainBar 
		var mainBar = [
			{text: 'New Termset', iconCls: 'dt-icon-add', handler:function(){this.fireEvent('new_termset')}, scope: this},
			{text: 'New Term', itemId: 'new_term', iconCls: 'dt-icon-add', hidden: true, handler:function(){this.fireEvent('new_term')}, scope: this},
		...
		//event handlers
		this.on('new_termset',function(){
			Desktop.AppMgr.display(
				'term_set_form',
				null, 
				{node: tree.getRootNode()}	//give tree's root node so that the form can refresh the entire tree on save
			);
		});
		
		this.on('new_term',function(){
			...
			Desktop.AppMgr.display(
				'term_form',
				null, 
				{node: node}	//give tree's root node so that the form can refresh the entire tree on save
			);
			

		});
		
		this.on('edit', function(node, e){
			var selected = tree.getSelectionModel().getSelectedNode();
			...
			if (selected.attributes.rdf_type == 'http://purl.org/linguistics/gold/TermSet') {
				...
				Desktop.AppMgr.display('term_form', selected.attributes.localname, {
					title: selected.attributes.label,
					node: selected.parentNode	//give tree's root node so that the form can refresh the entire tree on save
				});	
			}
