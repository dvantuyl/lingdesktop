	  ...
	  var ic = this.initialConfig; //configuration given to Desktop.App
		...
		var hasMeaningGrid = new Ontology.DropGrid({
		    ...
		    store: new Ext.data.JsonStore({
			    ...
			    root: 'data',
				idProperty: 'uri',
			    fields: ['rdftype', 'sid', 'uri', 'label', 'localname']
			}),
		...
		this.on('save',function(){
			...
			var save_config = {
				scope: this,
				params: {hasMeaning: Ext.encode(hasMeaningUris)}
			};
			if(ic.node){
				save_config.params.termset_uri = ic.node.attributes.uri
			}
			if(ic.instanceId){
