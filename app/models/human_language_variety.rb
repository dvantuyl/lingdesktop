class HumanLanguageVariety < RDF_Resource
  
  # Generates an escaped uri based on UUID timestamp algorithm
  #
  # @returns [String] uri
  def self.gen_uri_esc(args)
    if args.has_key?(:id)
      id = args[:id]
    else
    ...
    "http://purl.org/linguistics/lingdesktop/human_language_varieties/#{id}".uri_esc
  end  
  
  
  def self.type
    RDF_Resource.find_or_create(:uri_esc => RDF::GOLD.HumanLanguageVariety.uri_esc)
  end
  ...
  def self.create_in_context(context_node, args = {})
    ...
    node = HumanLanguageVariety.create(:uri_esc => self.gen_uri_esc(args))
